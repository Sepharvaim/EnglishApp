...
function generaHTML(array, sectionID) {
  // Utilizza template literals per creare una stringa HTML
  const swiperWrapper = document.getElementById(sectionID).querySelector(".swiper-wrapper");
  ...
  for (let i = 0; i < 25; i++) {
   ...
   <div class="back">
   <div class='sectorOne'>
   <p>Present Simple: ${array[i].infinitive}</p>
   <p>Past Simple: ${array[i].past_simple}</p>
   <p>Past Participle: ${array[i].past_participle}</p>
   <p>Third Person: ${array[i].third_person}</p>
   <p>Present Participle: ${array[i].present_participle}</p>
   </div>
   <div class='sectorTwo'></div>
   <div class='sectorThree'></div>
   <div class='sectorFour'>
  ...
  `;
    swiper_slide.innerHTML = htmlString;
    swiperWrapper.appendChild(swiper_slide);
  }
}


generaHTML(irregularVerb, "irregular_verb");
